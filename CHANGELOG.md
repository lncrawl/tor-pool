...
- **`EXIT_TTL` queues an instance for rotation once it has held one exit identity for too long**, defaulting to 3 hours. It catches the exit nothing reports: a relay a target site has quietly started scoring is not broken, so the failure ladder never hears about it and the instance keeps that IP indefinitely. **A queued instance rotates only once no session is pinned to it** — the clock never changes the exit IP under a caller that asked to be sticky — so it is a floor on an identity's age rather than a period, and `SESSION_TTL` is what frees an abandoned instance. One instance rotates at a time however many fall due together, and each instance reports `rotate_pending` so a queue held up by its sessions explains itself. Set `EXIT_TTL=0` for the previous behaviour, where an exit lasts until it fails or is rotated by hand.
